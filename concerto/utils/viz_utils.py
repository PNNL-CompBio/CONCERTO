...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
### Begin functions for Sankey/Alluvial Diagram format  ###
### ### ### ### ### ### ### ### ### ### ### ### ### ### ###

def get_org_names(flux_df):
    # Get list of org names from flux_df
    return [i for i in flux_df.index if not(i=='medium')] # Don't include the entry = 'medium'

def refine_flux_df(flux_df):
    org_names = get_org_names(flux_df)
    
    # Get list of external metabolites
    ex_metabs_cols = [c for c in flux_df.columns if (c.startswith('EX_') and c.endswith('_e'))]
    ex_metabs_names = [m[3:-2] for m in ex_metabs_cols]

    # Create df for only external metabolites (renamed) and organisms
    ex_flux_df = flux_df.loc[org_names][ex_metabs_cols]
    ex_flux_df.columns = ex_metabs_names

    return ex_flux_df

def set_x_pos():
    # Define x-positions for different node groupings in the diagram
    x_pos = {
...
def initialize_node_df(flux_df, x_pos):
    ...
    node_df = pd.DataFrame(columns=["Node_Idx", "Node_Name", "Type", "Xpos", "Ypos", "Color"])
    ...
    node_df = pd.concat([node_df,this_df], ignore_index=True)
    ...
    for this_m in flux_df.columns:
        ...
        for this_org in org_names:
            ...
            if (sum_good_flux >= 1) and (not np.isnan(this_val)) and (abs(this_val)>flux_threshold): 
                ...
                # Reaction from receiver-org to excreted-met: overall, only positive fluxes
...
def assign_colors_node_df(node_df, org_names, m_in_use, carrier_m, carrier_names):
    ...
    color_count = 0
    org_idx = [i for i,n in enumerate(node_df['Node_Name']) if (n in org_names)]
...
def get_y_pos(node_df, x_pos, carrier_m):
    ...
    for this_n_type in x_pos.keys():
        ...
        this_n_list = list(this_df['Node_Name'])
        ...
        if new_n_list[len(new_n_list)-1] != 'space':
            this_yPos_N += 1
            new_n_list.append('space')
...
def get_nodes_and_links(x_pos, flux_df, carrier_m, carrier_names, flux_threshold = 0.01):
    # Initialize node df
    node_df = initialize_node_df(flux_df, x_pos)

    # Refine node_df with links between nodes and update x_pos's
    node_df, m_in_use, links_df = refine_node_df(node_df, flux_df, x_pos, flux_threshold)

    # Assign colors to nodes
    org_names = get_org_names(flux_df)
    node_df = assign_colors_node_df(node_df, org_names, m_in_use, carrier_m, carrier_names)

    # Assign y-positions to nodes based on color & x-position groupings
    node_df = get_y_pos(node_df, x_pos, carrier_m)
...
# def assign_org2group():
#     return None

def create_fluxflow_viz(consort_model, flux_df, org2group_dict, flux_threshold = 0.01):
    # Update flux_df by filtering to external metabs and refining column names
    flux_df = refine_flux_df(flux_df)

    # Set x-coordinate info
    x_pos = set_x_pos()
    ...
    node_df, links_df = get_nodes_and_links(x_pos, flux_df, carrier_m, carrier_names, flux_threshold)
