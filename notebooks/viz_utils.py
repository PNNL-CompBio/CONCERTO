...
def ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold):
    ...
    org_ext_m = {
        1: {
            'name': org_list[0].id,#[0:2].upper(),
            'ext_m': get_ex_metabs(org_list[0]),
        },
        2: {
            'name': org_list[1].id,#[0:2].upper(),
            'ext_m': get_ex_metabs(org_list[1]),
        },
        3: {
            'name': org_list[2].id,#[0:2].upper(),
    ...
    if flux_df is not None:
        org_ext_m = filter_m_by_flux(org_ext_m, flux_df, exclude_0flux, flux_threshold)

    # Combine all external metabolites into a single list
    all_ext_m = set(org_ext_m[1]['ext_m']).union(org_ext_m[2]['ext_m']).union(org_ext_m[3]['ext_m'])

    # Two-way shared external metabolites
    twoway_ext_m = {}
    twoway_ext_m["1_2"] = [m for m in all_ext_m if (
                (m in org_ext_m[1]['ext_m']) and (m in org_ext_m[2]['ext_m']) and not (m in org_ext_m[3]['ext_m']))]
    twoway_ext_m["1_3"] = [m for m in all_ext_m if (
                (m in org_ext_m[1]['ext_m']) and (m in org_ext_m[3]['ext_m']) and not (m in org_ext_m[2]['ext_m']))]
    twoway_ext_m["2_3"] = [m for m in all_ext_m if (
                (m in org_ext_m[2]['ext_m']) and (m in org_ext_m[3]['ext_m']) and not (m in org_ext_m[1]['ext_m']))]

    # Three-way shared external metabolites
    threeway_ext_m = [m for m in all_ext_m if
                      ((m in org_ext_m[1]['ext_m']) and (m in org_ext_m[2]['ext_m']) and (m in org_ext_m[3]['ext_m']))]
    threeway_ext_m.sort()

    ### Create dict of shared external metabolites based on length and group type
    shared_ext_m = {'longest': {}, 'midlength': {}, 'shortest': {} }

    for i in range(1, len(org_ext_m) + 1):
        for j in range(i + 1, len(org_ext_m) + 1):
            k = [m for m in [1, 2, 3] if not (m == i) and not (m == j)][0]
            # Use correct keys for i,j,k pairs
            A = f"{i}_{j}"
            B = f"{i}_{k}" if i < k else f"{k}_{i}"
            C = f"{j}_{k}" if j < k else f"{k}_{j}"
            # sort alphabetically
            twoway_ext_m[A].sort()

            # identify longest and shortest lists
            if (len(twoway_ext_m[A]) >= len(twoway_ext_m[B])) and (len(twoway_ext_m[A]) >= len(twoway_ext_m[C])):
                shared_ext_m['longest']['2way_key'] = A
                shared_ext_m['longest']['orgs'] = [i, j]
                shared_ext_m['longest']['org_names'] = [org_ext_m[i]['name'], org_ext_m[j]['name']]
                shared_ext_m['longest']['ext_m'] = twoway_ext_m[A]
            if (len(twoway_ext_m[A]) <= len(twoway_ext_m[B])) and (len(twoway_ext_m[A]) <= len(twoway_ext_m[C])):
                shared_ext_m['shortest']['2way_key'] = A
                shared_ext_m['shortest']['orgs'] = [i, j]
                shared_ext_m['shortest']['org_names'] = [org_ext_m[i]['name'], org_ext_m[j]['name']]
                shared_ext_m['shortest']['ext_m'] = twoway_ext_m[A]

    # capture mid-length list
    mid_key = [k for k in twoway_ext_m.keys() if
               (not (k == shared_ext_m['shortest']['2way_key']) and not (k == shared_ext_m['longest']['2way_key']))][0]
    mid_i = int(mid_key[0])
    mid_j = int(mid_key[-1])
    shared_ext_m['midlength']['2way_key'] = mid_key
    shared_ext_m['midlength']['orgs'] = [mid_i, mid_j]
    shared_ext_m['midlength']['org_names'] = [org_ext_m[mid_i]['name'], org_ext_m[mid_j]['name']]
    shared_ext_m['midlength']['ext_m'] = twoway_ext_m[mid_key]

    # add three-way list
    shared_ext_m['threeway'] = {
        'orgs': [1, 2, 3],
        'org_names': [org_ext_m[1]['name'], org_ext_m[2]['name'], org_ext_m[3]['name']],
...
def get_met_pos_limits(shared_ext_m, org_pos, x_min, x_max, y_min, y_max, d_x, d_y):
    ...
    if (L > (row_col_max['longest']['r_max'] * row_col_max['longest']['c_max'])):
        print("WARNING: The LONGEST list of metabolites is too long for the limits set in place")
    row_cols['longest'] = {}
    row_cols['longest']['minR'] = numR2Cwidth * (1 + int(math.sqrt(L / numR2Cwidth))) if numR2Cwidth <= L else L
    ...
    pos_limits['longest']['bottom_row_end'] = {  # Bottom right end of longest list placement position
    ...
    pos_limits['shortest']['top_row_start_min'] = {  # below longest list AND (right of longest list OR older x-limit)
    ...
    pos_limits['shortest']['bottom_row_start_min']['x'] = pos_limits['shortest']['top_row_start_min']['x']

    pos_limits['threeway'] = {
        'top_row_start_min': {  # right of longest list; below longest_midlength organism
            'x': d_x + pos_limits['longest']['bottom_row_end']['x'],
            'y': d_y + org_pos['longest_midlength']['y']
        }
    }

    ### Calculate values for the Shortest two-way list: centered on bottom-side right position
    ...
    if (L > (row_col_max['shortest']['r_max'] * row_col_max['shortest']['c_max'])):
        print("WARNING: The SHORTEST list of metabolites is too long for the limits set in place")
    row_cols['shortest'] = {}
    row_cols['shortest']['minC'] = (1 + int(math.sqrt(L / numR2Cwidth))) if numR2Cwidth <= L else int(math.sqrt(L))
    ...
    if (radius > max_right_width or radius > max_left_width):
        print("NOTE: Adjusting SHORTEST list center position away from originally defined location")
        pos_limits['shortest']['bottom_row_center']['x'] = pos_limits['shortest']['top_row_start_min']['x'] + 0.5 * abs(
            pos_limits['shortest']['bottom_row_end_max']['x'] - pos_limits['shortest']['top_row_start_min']['x'])

    pos_limits['shortest']['bottom_row_start'] = {  # left of bottom-side right position
        'x': pos_limits['shortest']['bottom_row_center']['x'] - radius,
        'y': pos_limits['shortest']['bottom_row_center']['y']
    }
    pos_limits['shortest']['bottom_row_end'] = {  # right of bottom-side right position
        'x': pos_limits['shortest']['bottom_row_center']['x'] + radius,
        'y': pos_limits['shortest']['bottom_row_center']['y']
    }
    if (pos_limits['shortest']['bottom_row_start']['x'] < pos_limits['shortest']['bottom_row_start_min']['x']):
        print("WARNING: The SHORTEST list of metabolites will overlap with the LONGEST list of metabolites")
    if (pos_limits['shortest']['bottom_row_end']['x'] > pos_limits['shortest']['bottom_row_end_max']['x']):
        print("WARNING: The SHORTEST list of metabolites goes beyond the horizontal limit position from the right")
    pos_limits['shortest']['top_row_start'] = {  # directly above bottom_row_start position
        'x': pos_limits['shortest']['bottom_row_start']['x'],
        'y': pos_limits['shortest']['bottom_row_start']['y'] - d_y * (row_cols['shortest']['numR'] - 1)
    }
    if (pos_limits['shortest']['top_row_start']['x'] < pos_limits['longest']['bottom_row_end']['x']) or (
            pos_limits['shortest']['top_row_start']['y'] < pos_limits['longest']['bottom_row_end']['y']):
        print("WARNING: The SHORTEST list of metabolites may overlap with the LONGEST list of metabolites")
    pos_limits['shortest']['top_row_end'] = {  # directly above bottom_row_end position
        'x': pos_limits['shortest']['bottom_row_end']['x'],
        'y': pos_limits['shortest']['top_row_start']['y']
    }

    pos_limits['midlength']['bottom_row_end_max']['y'] = min(pos_limits['midlength']['bottom_row_end_max']['y'],
                                                             (pos_limits['shortest']['top_row_end']['y'] - d_y))

    ### Calculate values for the Midlength two-way list: centered on right-side top position
    ...
    if (L > (row_col_max['midlength']['r_max'] * row_col_max['midlength']['c_max'])):
        print("WARNING: The MIDLENGTH list of metabolites is too long for the limits set in place")
    row_cols['midlength'] = {}
    row_cols['midlength']['numC'] = min((1 + int(L / row_col_max['midlength']['r_max'])),
    ...
    if (radius > max_top_width or radius > max_bottom_width):
        print("NOTE: Adjusting MIDLENGTH list center position away from originally defined location")
        pos_limits['midlength']['right_col_center']['y'] = pos_limits['midlength']['top_row_end_min']['y'] + 0.5 * abs(
            pos_limits['midlength']['bottom_row_end_max']['y'] - pos_limits['midlength']['top_row_end_min']['y'])
    ...
    if (L > (row_col_max['threeway']['r_max'] * row_col_max['threeway']['c_max'])):
        print("WARNING: The THREE-WAY list of metabolites is too long for the limits set in place")
    row_cols['threeway'] = {}
    row_cols['threeway']['minR'] = numR2Cwidth * (1 + int(math.sqrt(L / numR2Cwidth))) if numR2Cwidth <= L else L
    ...
    pos_limits['threeway']['center'] = {
        'x': pos_limits['threeway']['top_row_start_min']['x'] + 0.5 * abs(
            pos_limits['threeway']['bottom_row_end_max']['x'] - pos_limits['threeway']['top_row_start_min']['x']),
...
def create_flux_dict_info(model_dict, flux_df, exclude_0flux, flux_threshold):
    excluded_reactions = []
    excluded_metabolites = []
    this_flux_dict = {}

    for r in model_dict['reactions']:
        print(r['id'])
        r_m = r['id'][:-6]
        org_id = r['id'][-5:]
    ...
    flux_dict_info = {
        ...
        'excluded_met': excluded_metabolites,
        'excluded_reacts': excluded_reactions
    }
...
def create_flux_map(model_name, ext_met_df, flux_dict=None):
    ...
    for m in ex_met_exchange_map['metabolites']:
        ...
        m_name = f"{m['id']}_" if m['id']==m['compartment'] else m['notes']['map_info']['display_name']
    ...
    display("Showing map corresponding to the following file: " + updated_fname)
...
# Function to produce reorganized visualization for shared external metabolites for 3-organisms
def make_3organism_extmetab_viz(org_list, model_name,
    ...
    if multi_org_model == None:
        multi_org_model = make_met_exchange_model_only_shared_external_mets(org_list, model_name)
    ...
    #### ==> Careful not to omit the original organisms which are represented as metabolite nodes

    #### shared_ext_m, excluded_met = ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold)
    # shared_ext_m = ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold)
    if reorg_after_filter:
        # Filter out using flux data first, then reorganize the node coordinates
        shared_ext_m = ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold)
    else:
        # First create model/dict using all metabolites, then generate the d3flux map using exclusion info (later)
        shared_ext_m = ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold)

        # if not reorg_after_filter:
    #     shared_ext_m = ex_metab_reorg(org_list, flux_df=None, exclude_0flux=False, flux_threshold=None)
    #     # shared_ext_m = ex_metab_reorg(org_list, flux_df, exclude_0flux, flux_threshold)
    # else:
    #     shared_ext_m = ex_metab_reorg(org_list, flux_df=None, exclude_0flux=False, flux_threshold=None)
    ...
    row_cols, pos_limits = get_met_pos_limits(shared_ext_m, org_pos, x_min, x_max, y_min, y_max, d_x, d_y)

    # Get coordinate positions for each external metabolite node, including organism nodes
    ...
    if reorg_after_filter:
        ...
        flux_dict_info = create_flux_dict_info(model_dict, flux_df, exclude_0flux, flux_threshold)
        print(flux_dict_info)
        # fluxmap_dict["map"] = d3f.flux_map(fluxmap_dict['model'],
        #                                    excluded_reactions=flux_dict_info['excluded_reacts'],
        #                                    excluded_metabolites=flux_dict_info['excluded_met'],
        #                                    flux_dict=flux_dict_info['flux_dict']
        #                                    )
    else:
        fluxmap_dict = create_flux_map(model_name, ext_met_df)
        flux_dict_info = create_flux_dict_info(model_dict, flux_df, exclude_0flux, flux_threshold)
